...
public class RomePuzzle extends Fifteen implements Verify,Outmod{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        currentLevel = 3;
        passedLevel = 4;
        titleLevel.setText(getResources().getText(R.string.level_three));
        ...
        mChronometer.start();
        showMessage(getResources().getString(R.string.name_level_three));
    }
    ...
    public boolean checkOverGame() {
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                ...
                if (button == buttons[0][0] && getTitleButton(button).equals("I")) { count++; }
                if (button == buttons[0][1] && getTitleButton(button).equals("II")) { count++; }
                if (button == buttons[0][2] && getTitleButton(button).equals("III")) { count++; }
                if (button == buttons[0][3] && getTitleButton(button).equals("IV")) { count++; }
                if (button == buttons[1][0] && getTitleButton(button).equals("V")) { count++; }
                if (button == buttons[1][1] && getTitleButton(button).equals("VI")) { count++; }
                if (button == buttons[1][2] && getTitleButton(button).equals("VII")) { count++; }
                if (button == buttons[1][3] && getTitleButton(button).equals("VIII")) { count++; }
                if (button == buttons[2][0] && getTitleButton(button).equals("IX")) { count++; }
                if (button == buttons[2][1] && getTitleButton(button).equals("X")){ count++; }
                if (button == buttons[2][2] && getTitleButton(button).equals("XI")){ count++; }
                if (button == buttons[2][3] && getTitleButton(button).equals("XII")){ count++; }
                if (button == buttons[3][0] && getTitleButton(button).equals("XIII")){ count++; }
                if (button == buttons[3][1] && getTitleButton(button).equals("XIV")){ count++; }
                if (button == buttons[3][2] && getTitleButton(button).equals("XV")){ count++; }
            }
        }
    ...
    public void nextLevel() {
        Intent intent_cont = new Intent(getApplicationContext(), AlphabetPuzzle.class)
