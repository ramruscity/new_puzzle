...
public class HexadecimalPuzzle extends Fifteen implements Verify,Outmod {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        titleLevel.setText(getResources().getText(R.string.level_ten));
        ...
        mChronometer.start();
        showMessage(getResources().getString(R.string.name_level_ten));
    }
    ...
    public boolean checkOverGame() {
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                ...
                if (button == buttons[2][1] && getTitleButton(button).equals("A")){ count++; }
                if (button == buttons[2][2] && getTitleButton(button).equals("B")){ count++; }
                if (button == buttons[2][3] && getTitleButton(button).equals("C")){ count++; }
                if (button == buttons[3][0] && getTitleButton(button).equals("D")){ count++; }
                if (button == buttons[3][1] && getTitleButton(button).equals("E")){ count++; }
                if (button == buttons[3][2] && getTitleButton(button).equals("F")){ count++; }
            }
        }
    ...
    public void nextLevel() {
        Intent intent_cont = new Intent(getApplicationContext(), MagicQuadrantPuzzle.class)
