...
public class AlphabetPuzzle extends Fifteen implements Verify,Outmod {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        titleLevel.setText(getResources().getText(R.string.level_four));
        getBestResult(currentLevel);
        initArray();
        generateArray();
        paintTable();
        generationButton(4000);
        setListenersOnButtons();
        mChronometer.setBase(SystemClock.elapsedRealtime());
        mChronometer.start();
        showMessage(getResources().getString(R.string.name_level_four));
    }
    ...
    public boolean checkOverGame() {
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                ...
                if (button == buttons[0][0] && getTitleButton(button).equals("A")) { count++; }
                if (button == buttons[0][1] && getTitleButton(button).equals("B")) { count++; }
                if (button == buttons[0][2] && getTitleButton(button).equals("C")) { count++; }
                if (button == buttons[0][3] && getTitleButton(button).equals("D")) { count++; }
                if (button == buttons[1][0] && getTitleButton(button).equals("E")) { count++; }
                if (button == buttons[1][1] && getTitleButton(button).equals("F")) { count++; }
                if (button == buttons[1][2] && getTitleButton(button).equals("G")) { count++; }
                if (button == buttons[1][3] && getTitleButton(button).equals("H")) { count++; }
                if (button == buttons[2][0] && getTitleButton(button).equals("I")) { count++; }
                if (button == buttons[2][1] && getTitleButton(button).equals("J")){ count++; }
                if (button == buttons[2][2] && getTitleButton(button).equals("K")){ count++; }
                if (button == buttons[2][3] && getTitleButton(button).equals("L")){ count++; }
                if (button == buttons[3][0] && getTitleButton(button).equals("M")){ count++; }
                if (button == buttons[3][1] && getTitleButton(button).equals("N")){ count++; }
                if (button == buttons[3][2] && getTitleButton(button).equals("O")){ count++; }
            }
        }
    ...
    public void nextLevel() {
        Intent intent_cont = new Intent(getApplicationContext(), HellenicPuzzle.class)
